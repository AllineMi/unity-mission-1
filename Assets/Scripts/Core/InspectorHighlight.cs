        ...
        foreach (var unmaskedView in unmaskedViews)
        {
            foreach (var view in GetMatchingViews(unmaskedView, allViews, viewsWithWindows))
            {
                ...
                if (!result.TryGetValue(view, out maskViewData))
                {
                    ...
                    {
                        rects = new List<Rect>(8), maskType = unmaskedView.m_MaskType,
...
//                        var window = EditorWindow.GetWindow(targetEditorWindowType);
                    var window = Resources.FindObjectsOfTypeAll(targetEditorWindowType).Cast<EditorWindow>().ToArray()
                        .FirstOrDefault();
    ...
    [SerializedTypeGUIViewFilter] [SerializeField]
    private SerializedType m_ViewType = new SerializedType(null);

    [SerializedTypeFilter(typeof(EditorWindow))] [SerializeField]
    ...
    [SerializeField] private MaskType m_MaskType = MaskType.FullyUnmasked;

    [SerializeField] private MaskSizeModifier m_MaskSizeModifier = MaskSizeModifier.NoModifications;

    [SerializeField] private List<GUIControlSelector> m_UnmaskedControls = new List<GUIControlSelector>();
